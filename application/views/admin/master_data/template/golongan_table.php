<html>	
	...
	<body>
				...
				<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb overviewtable2">
					...
					<tbody>
						{{#if results}}
							{{#each results}}
							<tr id="{{id_golongan}}">
								<td class="chkbox"><input type="checkbox" class="chkbox-input" value="{{id_golongan}}"/></td>
								<td>{{kode_golongan}}</td>                                                                                   
								<td>{{nama}}</td>                                                                                  
								<td>
									{{#ifCond status '==' 1}}
		...
		<form method="POST" action="#" id="form-hidden-filter" class="hidden">
			<input type="hidden" name="kode_golongan" id="f_kode_golongan" value="<?php echo $kode_golongan?>"/>
		...
		<script>
			...
			var url = "admin/master_data/golongan_table/";	
			
			function populate_filter(data){
				$('#f_kode_golongan').val(data.kode_golongan);
